...
public class InventoryService implements GameService<Inventory> {
    ...
    public Iterable<Inventory> findAll(@NonNull Sort sort) {
        return repository.findAll(sort);
    }

    @NonNull
    @CachePut(value = cacheName, cacheManager = cacheManagerName)
    public Inventory save(@NonNull Inventory entity) {
        return repository.save(entity);
    }

    @CacheEvict(value = cacheName, cacheManager = cacheManagerName)
    public void delete(@NonNull Inventory entity) {
        repository.delete(entity);
    }

    @NonNull
    @Cacheable(value = cacheName, cacheManager = cacheManagerName)
    public Page<Inventory> findAll(@NonNull Pageable pageable) {
        return repository.findAll(pageable);
    }

    @NonNull
    @CachePut(value = cacheName, cacheManager = cacheManagerName)
    public <S extends Inventory> Iterable<S> saveAll(@NonNull Iterable<S> entities) {
        return repository.saveAll(entities);
    }

    @NonNull
    public Optional<Inventory> findById(@NonNull Integer integer) {
    ...
    @NonNull
    public Iterable<Inventory> findAllById(@NonNull Iterable<Integer> integers) {
    ...
    @CacheEvict(value = cacheName, cacheManager = cacheManagerName)
    public void deleteAll(@NonNull Iterable<? extends Inventory> entities) {
    ...
    @NonNull
    @Cacheable(value = cacheName, cacheManager = cacheManagerName)
    public Optional<Inventory> findOne(@NonNull Predicate predicate) {
        return repository.findOne(predicate);
    }

    @NonNull
    @Cacheable(value = cacheName, cacheManager = cacheManagerName)
    public Iterable<Inventory> findAll(@NonNull Predicate predicate) {
        return repository.findAll(predicate);
    }

    @NonNull
    @Cacheable(value = cacheName, cacheManager = cacheManagerName)
    public Iterable<Inventory> findAll(@NonNull Predicate predicate, @NonNull Sort sort) {
        return repository.findAll(predicate, sort);
    }

    @NonNull
    @Cacheable(value = cacheName, cacheManager = cacheManagerName)
    public Iterable<Inventory> findAll(@NonNull Predicate predicate, @NonNull OrderSpecifier<?>... orders) {
        return repository.findAll(predicate, orders);
    }

    @NonNull
    @Cacheable(value = cacheName, cacheManager = cacheManagerName)
    public Iterable<Inventory> findAll(@NonNull OrderSpecifier<?>... orders) {
        return repository.findAll(orders);
    }

    @NonNull
    @Cacheable(value = cacheName, cacheManager = cacheManagerName)
    public Page<Inventory> findAll(@NonNull Predicate predicate, @NonNull Pageable pageable) {
    ...
    @NonNull
    @Cacheable(value = cacheName, cacheManager = cacheManagerName)
    public <S extends Inventory, R> R findBy(
